    ...
    $("#search-btn").click(function () {
        ...
        ajax.done(function(res){
            ...
            for(let i = 0; i < res.length; i++) {
                let customer = res[i];
                table +=  `<tr id="row_${i}"><td>${customer.id}</td><td>${customer.name}</td><td>${customer.address}</td><td>${customer.email}</td><td>${customer.password}</td><td>${customer.phone_number}</td></tr>`;
                if (i == 0) {
